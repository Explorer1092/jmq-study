package com.jimingqiang.study.common.snowflake;
...
public class SnowflakeUtil {
    ...
    public static synchronized SnowflakeUtil getInstance() {
        if (null == snowflakeUtil) {
            snowflakeUtil = new SnowflakeUtil(0L, 0L);
        }
    ...
    private SnowflakeUtil(long workerId, long datacenterId) {
        this.datacenterId = new Random().nextInt(MAX) % (MAX - MIN + 1) + MIN;
        this.workerId = new Random().nextInt(MAX) % (MAX - MIN + 1) + MIN;
    }
...
//        System.out.println(ip);
        ...
        for (int idex = 0; idex < 1100; idex++) {
            ...
            for (int j = 0; j < 510; j++) {
                SnowflakeUtil su = SnowflakeUtil.getInstance();
                map.put(su.nextId(), "2");
                count++;
            }
            count++;
